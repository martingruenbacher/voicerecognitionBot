...
class WeatherReport():
    ...
    def get_weather(self, name="", location="Desselbrunn"):
        try:
            ...
            if self.cnt_failed > 10:
                cwd = os.getcwd()
                f = open(cwd + "\\Fiona2.1\\resources\\weather\\reboot_flag.txt", "w")
                f.write("True")
                f.close()
        ...
        cwd = os.getcwd()
        f = open(cwd + "\\Fiona2.1\\resources\\weather\\weather_report.txt", "w+")
        f.write(text)
        f.close()
    ...
    def get_weather_prediction(self, name=""):
        ...
        #-->os.system("sudo pkill chromium")
            cwd = os.getcwd()
            f = open("\\Fiona2.1\\resources\\weather\\master.log", "a")
